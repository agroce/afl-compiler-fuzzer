...
`afl-fuzz-compiler-pure` is similar, except even more aggressive about using the language-focused fuzzing, possibly too much so (not yet tested).  Usage of it or `afl-fuzz-compiler` is exactly like that for normal AFL; just use our executable instead of `afl-fuzz`.
